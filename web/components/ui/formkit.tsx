...
// Shared form primitives for the console's modals, ported verbatim from Lumen's
// formkit (itself ported from QuartzFire) so a field in Flux and a field in the
// other consoles are the same control: the same padding, the same border that
// turns accent on focus, monospace for anything an operator reads off a
// network diagram.
...
export function SelectInput({
  ...
  return (
    <select
      ...
      disabled={disabled}
      onChange={(e) => onChange(e.target.value)}
      className={`${inputCls} cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed`}
      style={invalid ? (mono ? monoInvalidSt : invalidSt) : base}
      onFocus={focusBorder}
      onBlur={(e) => {
        e.currentTarget.style.borderColor = invalid ? 'var(--qz-danger)' : 'var(--qz-border)';
...
/// A scrolling box of checkable rows — the ports a group collects.
