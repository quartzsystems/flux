...
/// Per-row edit/delete for the config tables, ported verbatim from Lumen
/// (which ports it from QuartzFire). Delete asks for inline confirmation
/// before applying.
///
/// Either control can be switched off with a reason: the tables carry rows the
/// signed-in role cannot change and rows something else depends on, and a
/// control that says why it is off is worth more than one that is silently
/// grey.
export function RowActions({
  ...
  /** Accessible name of the row, e.g. `flow Flow1`. */
  label: string;
  onEdit: () => void;
