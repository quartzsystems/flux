...
/// The console's one table, ported verbatim from Lumen (which ports it from
/// QuartzFire): a search box and drop-down filters over the top, sortable
/// headers the operator can hide, reorder, and drag to resize, click/shift/ctrl
/// row selection, and a row count. The layout is remembered per `storageKey`,
/// so an arrangement survives a reload.
...
export function DataTable<T>({
  ...
  const visibleCols = useMemo(
    () =>
      orderedKeys
        .map((k) => byKey.get(k))
        .filter((c): c is Column<T> => c !== undefined && !hidden.has(c.key)),
  ...
  const onResizeDown = (e: React.MouseEvent, index: number) => {
    ...
    const th = (e.currentTarget.parentElement as HTMLElement).closest('th');
  ...
  useEffect(() => {
    const move = (e: MouseEvent) => {
      ...
      setWidths((w) => ({
        ...w,
        [r.leftKey]: r.startLeft + delta,
        [r.rightKey]: r.startRight - delta,
      }));
    };
    const up = () => {
  ...
  const onRowMouseDown = (e: React.MouseEvent, index: number) => {
    ...
    dragBaseRef.current = new Set();
    const id = displayedIds[index];
    setSelected(new Set(id === undefined ? [] : [id]));
  };
  ...
  // away since it was ticked — a port unplugged, a flow deleted — must not be
  ...
  // The checkbox and actions cells hold controls a double-click must not fall
  // through — double-clicking Start is two starts, not "open the row". But
  // the cells are wider than their controls, and a double-click that lands on
  // the empty part of one is an operator aiming at the row: honour it.
  const onCellDoubleClick = (e: React.MouseEvent, row: T) => {
    e.stopPropagation();
    const target = e.target as HTMLElement;
    if (onRowOpen && !target.closest('button, a, input, select, label')) onRowOpen(row);
  };
            ...
            {menuOpen && (
              <div className="menu">
                ...
                <button
                  ...
                  className="menu-item menu-item-row menu-item-quiet"
                >
        ...
        >
          ...
          <thead>
            <tr>
              ...
              {actions && (
                <th style={{ width: actionsWidth }} className="right qz-sticky-actions">
                  Actions
                </th>
              )}
            </tr>
          ...
          <tbody>
            {displayed.length === 0 ? (
              <tr>
                <td colSpan={colSpan} className="empty" style={{ cursor: 'default' }}>
                  {emptyMessage}
                </td>
              </tr>
            ) : (
              displayed.map((row, index) => {
                const id = displayedIds[index];
                const isSel = id !== undefined && selected.has(id);
                return (
                  <tr
                    key={id}
                  ...
                  >
                    ...
                    {actions && (
                      <td
                        ...
                        className="right qz-sticky-actions"
                      >
                        {actions(row)}
                      </td>
                    )}
