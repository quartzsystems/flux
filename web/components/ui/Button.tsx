...
/// The console's one button, ported verbatim from Lumen / Quartz Command so all
/// three products' toolbars are the same control. A disabled button carries no
/// tooltip of its own — wrap it in a `<span title="…">` when the reason it is
/// off needs saying.
...
const kindStyles: Record<ButtonKind, string> = {
  ...
  danger: 'bg-[var(--qz-danger)] text-white hover:opacity-90',
};
...
export function IconButton({
  ...
  return (
    <button
      ...
      title={label}
      aria-label={label}
      className="w-8 h-8 rounded-md grid place-items-center bg-transparent text-[var(--qz-fg-3)] border border-transparent hover:bg-[var(--qz-surface)] hover:text-[var(--qz-fg-1)] hover:border-[var(--qz-border)] transition-all duration-[120ms] cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
    >
      <Icon size={16} />
    </button>
  );
